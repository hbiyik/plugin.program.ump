...
class cInputWindow(xbmcgui.WindowDialog):
    def __init__(self, *args, **kwargs):
        bg_image = os.path.join(addon_dir, 'resources', 'skins', 'Default', 'media', 'DialogBack2.png')
        check_image = os.path.join(addon_dir, 'resources', 'skins', 'Default', 'media', 'checked.png')
        ...
        self.addControl(self.strActionInfo)
        self.addControl(self.strSiteInfo)
...
class UnCaptchaReCaptcha:
    def processCaptcha(self, key, lang, opener,ua,sitemsg):
        headers = {'Referer': 'https://www.google.com/recaptcha/api2/demo', 'Accept-Language': lang,"User-agent":ua}
        opener.addheaders=headers.items()
        html = opener.open('http://www.google.com/recaptcha/api/fallback?k=%s' % (key))
        html = html.read()
        token = ''
        iteration = 0
        while True:
            ...
            oSolver = cInputWindow(captcha=captcha_imgurl, msg=message, iteration=iteration, sitemsg=sitemsg)
            captcha_response = oSolver.get()
            if not captcha_response:
                break
            response="c=%s"%str(cval)
            for captcha in captcha_response:
                response=response+"&response=%s"%str(captcha)
            opener.addheaders=headers.items()
            html = opener.open("http://www.google.com/recaptcha/api/fallback?k=%s" % (key), data=response)
            html = html.read()
        return token